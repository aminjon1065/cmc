...
**All items done (P4.1–P4.8 / ADR-0060…0066; P4.4 Mobile companion now ✅ as a PWA / ADR-0075, 2026-06-03).** Horizon P4 complete.
...
### P4.4 — Mobile companion ✅ COMPLETED (2026-06-03 — PWA; was DEFERRED, now resolved)
**Decision (2026-06-03):** AskUserQuestion chose **PWA on the existing Next.js** (not React Native) — single codebase, installable, offline-capable, **no app store** (sovereign / air-gap friendly; RN needs store/sideload, hostile to air-gap), reuses the whole web+BFF+auth. First slice = **PWA foundation + offline incident capture**. (1) **PWA foundation:** `app/manifest.ts` → `/manifest.webmanifest` (installable, `display:standalone`), `public/icon.svg` + `viewport.themeColor`, **conservative `public/sw.js`** (precache offline shell; navigations network-first → `/offline` fallback; static cache-first; API/RSC untouched so Next data never stale), `app/offline/page.tsx`, `components/pwa-register.tsx` (registers SW + online/offline & queue-depth badge + **drains the queue on reconnect**). (2) **Offline incident capture:** `lib/offline-incidents.ts` (IndexedDB queue) + the create-incident form persists the draft when `navigator.onLine` is false **or** the server action throws mid-submit, and `PwaRegister` replays each draft on reconnect through the **same `createIncidentAction`** (identical BFF + RLS + audit path); drops on server-reached, retries on pure network failure. Validation: web **tsc ✓ / lint ✓ / `next build` ✓** (29/29 routes incl. generated `/manifest.webmanifest`, `/offline`); `output:"standalone"` so `next start` smoke N/A (deploy serves `.next/standalone/server.js`); auth/middleware untouched → 307/401 unchanged; backend untouched. **ADR-0075.** **Boundary:** install prompt + SW offline navigation + the IndexedDB→reconnect→sync flow = manual browser live-smoke (DevTools Offline). `apps/web/{public/sw.js,public/icon.svg,src/app/manifest.ts,src/app/offline,src/components/pwa-register.tsx,src/lib/offline-incidents.ts}`. Follow-ons: offline for more modules + read offline-first cache; Web Push (SW now in place) + background-sync; encrypt queued drafts; PNG icon set.
...
### P5.1 — LLM gateway (self-hosted) ✅ COMPLETED (2026-06-03)
**Decisions (2026-06-03):** a **gated OpenAI-compatible seam** (vLLM / Ollama / llama.cpp over HTTP), not a vendor SDK; the real GPU serving is a **compose-profile + live boundary** (no GPU in CI). Gated `LLM_PROVIDER` (Noop / `OpenAiCompatLlmProvider` `fetch` → `{LLM_BASE_URL}/v1/chat/completions`; `LLM_ENABLED`). `LlmService`: **per-tenant Redis rate-limit** (`cmc:llm:rl:{tenant}`, `LLM_RATE_LIMIT_PER_MIN` → 429) + **metadata-only audit** (`llm.complete`: model/tokens/latency/outcome; raw prompts only with `LLM_LOG_PROMPTS`; failure audits durable); **503** disabled / **502** provider error / **400** bad request. `POST /v1/llm/complete` (`@Authorize llm:use`, granted operator + tenant_admin; non-streaming MVP). Contracts `llm.ts`; config `LLM_*`; no DB (config-only). e2e `llm` **5/5** (Noop factory when disabled; completion + **metadata-only audit asserting the raw prompt is absent from `audit_log`**; 403 / 400 / 429 — faked provider). Suite **63 suites / 447 tests**, zero regressions; `tsc`/eslint clean. **ADR-0067.** **Boundary:** real vLLM/GPU serving = manual live-smoke. `apps/api/src/modules/llm/`. **Substrate for P5.4 (RAG) / P5.5 (copilots).** Follow-ons: SSE streaming; token-bucket + per-user limits; embeddings (P5.2); model routing/guardrails; vLLM compose profile + GPU manifest.

### P5.2 — Vector pipeline ✅ COMPLETED (2026-06-03)
**Reframed (single-site / PostGIS-image reality):** the plan named **Qdrant** + "migrate or supplement pgvector". Reality: the dev/test Postgres image is **PostGIS**, and stacking the **pgvector** extension on it (or a `CREATE EXTENSION vector` migration) is non-trivial and would break the suite if absent; a Qdrant container is a heavy new gated dependency. So the first cut stores vectors **in Postgres as a `jsonb` column** (no extension, no new container), behind the gated AI substrate — **pgvector ANN index / Qdrant is the scale follow-on** that swaps in without changing the pipeline.

**Decisions (2026-06-03):** AskUserQuestion locked **"Postgres (vector as column) + gated-seam"**, embeddings **"via the P5.1 LLM gateway (`/v1/embeddings`)"**, indexer **"best-effort inline in `DocumentsService` + reindex"**. (1) **Embeddings via the gateway:** `LlmProvider` gained `embed(texts, model)` (OpenAI-compatible `/v1/embeddings`; `LLM_EMBED_MODEL` default `bge-m3`) on Noop + `OpenAiCompatLlmProvider` — one AI substrate serves chat *and* embeddings, faked in tests. (2) **Vector store:** `document_embeddings` (migration 0041, **RLS** two-GUC): `tenant_id`, `document_id` (FK cascade, **unique** → one vector/doc, upsert), `model`, `dims`, `embedding` **`jsonb`**, timestamps. No pgvector extension → migration safe everywhere. (3) **Gated best-effort indexer:** `VectorIndexService` active only when **`VECTOR_ENABLED` AND the LLM provider is up** (so it's a no-op in dev/test/CI where the provider is a noop). Rides the document lifecycle — `DocumentsService.indexDoc` (embed + `onConflictDoUpdate` upsert) on finalize, `removeDocument` on delete, both **best-effort** (never block/fail the document op, mirrors the OpenSearch P3.6 indexer). `reindexAll` backfills; `status` reports `{active, indexed}`. `VectorController`: `GET /v1/vector/status` (`document:read`) + `POST /v1/vector/reindex` (`document:write`). Contracts `vector.ts`; config `VECTOR_ENABLED` + `LLM_EMBED_MODEL`. e2e `vector` **4/4** (reindex embeds available docs asserting dims/model/the stored vector; idempotent upsert — no dup rows; status; RBAC 403 — faked provider). Suite **64 suites / 451 tests**, zero regressions; `tsc`/eslint clean. **ADR-0068.** **Boundary:** real embeddings via a live OpenAI-compatible endpoint = manual live-smoke. `apps/api/src/modules/vector/`. **Substrate for P5.3 (semantic search) / P5.4 (RAG).** Follow-ons: pgvector ANN index / Qdrant at scale; async (NATS) embedding worker; embed incidents + cases; chunk long docs; full-content extraction (P5.6).

### P5.3 — Semantic search ✅ COMPLETED (2026-06-03)
**Decisions (2026-06-03):** AskUserQuestion locked **brute-force cosine over the `jsonb` vectors** (no pgvector — consistent with P5.2; ANN = scale follow-on), **RRF fusion reusing the P3.7 pattern**, and **fold the semantic lane into `/v1/search`** (one surface, not a separate endpoint). Vectors are **documents-only** today (P5.2 deferred incident/case embeddings), so the lane augments the documents portion; incidents/cases stay keyword. (1) **Brute-force kNN:** pure `cosineSimilarity(a,b)` (`vector/cosine.ts`; returns 0 for non-comparable — mismatched dims / zero magnitude — so a bad embedding can't out-rank a real match) + `VectorIndexService.similar(query, cap)` (gated; embeds the query via the LLM gateway, RLS-reads the tenant's vectors, scores equal-dim rows, drops non-positive, returns top-`cap` `{id,score}[]` — **symmetric with the OpenSearch lane**). (2) **Fused into `/v1/search`:** `SearchService` resolves the vector lane before the request tx, hydrates its ids through the **same `hydrateDocHits`** as keyword (so **folder-access + RLS + `ready`-only + `deleted_at` filters apply identically** → permission-aware), and adds it to the fuser. `fuse()` now **sums RRF per `(type,id)`** → a doc in both lanes is **deduped into one boosted hit** reported `source:"hybrid"` (`SearchSource` gained `vector`+`hybrid`); disjoint lanes are unchanged, so P3.7 behaviour is preserved. (3) **P5.2 bug fixed:** `reindexAll` filtered `status="available"`, but docs finalize to **`ready`** (live `indexDoc` + search hydrate use `ready`; the P5.2 e2e masked it) → reindex would embed nothing + hits would be dropped; changed to `ready`. e2e `search-semantic` **7/7** (pure cosine; vector-only doc surfaces as `vector`; both-lane doc deduped to `hybrid` w/ boosted score; soft-deleted nearest-neighbour dropped by hydrate; no `document:read` → empty — faked provider, OpenSearch off so keyword=FTS). Suite **65 suites / 458 tests**, zero regressions; `tsc`/eslint clean. **ADR-0069.** **Boundary:** real embeddings = manual live-smoke. `apps/api/src/modules/{vector,search}/`. **Substrate for P5.4 (RAG).** Follow-ons: pgvector ANN / Qdrant at scale; embed incidents+cases; chunk long docs; web `hybrid`/`vector` source badge.

### P5.4 — RAG framework ✅ COMPLETED (2026-06-03)
**Decisions (2026-06-03):** AskUserQuestion locked **reuse `/v1/search` (P5.3 hybrid, permission-aware)** for retrieval, **strict grounding + inline `[n]` citations**, a **new `POST /v1/rag/ask`** surface, and **audit = metadata + cited source ids** (provenance, no raw text — mirrors P5.1). RAG is a **composition seam — no new model/store/migration.** `RagService.ask`: (1) 503 if the LLM provider is inactive; (2) retrieve via `SearchService.search` (hits already RBAC + folder-access filtered → RAG inherits permission-aware retrieval, grounds only in what the caller may read); (3) **no sources → honest "I could not find an answer…", `grounded:false`, zero usage, NO LLM call** (still audited); (4) assemble numbered context from each hit's `title`+`snippet` (documents = name+description until P5.6), bounded by `RAG_CONTEXT_CHAR_BUDGET`; (5) generate via `LlmService.complete` (inherits per-tenant **rate-limit**, **502** provider mapping, `llm.complete` metadata audit) with a **strict-grounding** system prompt at `temperature 0`; (6) parse `[n]` → distinct in-range `citations[]` ({type,id,title}), `grounded` = ≥1 cited; (7) **`rag.ask` audit** = model/latency/retrieved+cited counts/usage + **`citedSources` (type+id) provenance**; raw question/answer only under `LLM_LOG_PROMPTS`; failure audits durable. `POST /v1/rag/ask` (`@Authorize llm:use`, Zod→400) → `{answer, citations[], grounded, model, usage}`. Contracts `rag.ts`; config `RAG_TOP_K` (5) + `RAG_CONTEXT_CHAR_BUDGET` (6000); no migration. e2e `rag` **6/6** (grounded + `[n]`→id citation; honest no-answer with **no LLM call**; **metadata-only audit** — cited ids present, raw question absent; 403 без llm:use; 400 empty; **503** disabled — faked provider, real hybrid retrieval). Suite **66 suites / 464 tests**, zero regressions; `tsc`/eslint clean. **ADR-0070.** **Boundary:** real generation = manual live-smoke. `apps/api/src/modules/rag/`. **Substrate for P5.5 (copilots).** Follow-ons: SSE streaming; RAG-specific rate limit; full-content context + chunking (P5.6); ground in incidents/cases bodies; web ask-UI with rendered citations.

### P5.5 — Per-module copilots ✅ COMPLETED (2026-06-03 — framework + Incidents; GIS/Docs/Workflow follow-on)
**Decisions (2026-06-03):** AskUserQuestion locked **read-only advisory** (no mutations; tool-calling = follow-on), **framework + Incidents copilot first** (GIS/Docs/Workflow follow-ons behind the same surface), **module-scoped RAG + optional `resourceId` record anchor**, and a **unified `POST /v1/copilot/ask`**. A copilot is a **composition seam — no new model/store/migration/permission.** `CopilotService` holds a per-module registry `{readPermission, domainTypes, systemPrompt, loadAnchor}`; first entry `incidents` (`incident:read`, domain `["incident"]`, EOC-operator persona, anchor via the access-checked `IncidentsService.getDetail`). Flow: 503 if provider inactive → resolve perms (**`llm:use` but no module read-perm → honest no-answer, no leak**) → optional `resourceId` anchor (pinned first, loaded only if accessible) → module-scoped retrieval via `SearchService` filtered to `domainTypes` → merge+dedupe (anchor first) → shared `assembleContext` → no sources ⇒ honest no-answer with **no LLM call** → else `LlmService.complete` (inherits rate-limit/502/`llm.complete` audit) with the module strict-grounding prompt @ temp 0 → `resolveCitations` → **`copilot.ask` audit** (module, `anchorResourceId?`, cited-id provenance, counts, usage; raw Q/A only under `LLM_LOG_PROMPTS`; failure durable). **DRY:** `assembleContext` + `resolveCitations` extracted to `rag/grounding.ts`, shared by RAG + copilot (RAG e2e re-confirms `RagService`). `POST /v1/copilot/ask` (`@Authorize llm:use`, Zod→400) → `{answer, citations[], grounded, model, usage}`. Contracts `copilot.ts` (`COPILOT_MODULES=["incidents"]`, citations reuse `RagCitationSchema`); reuses `RAG_TOP_K`+`RAG_CONTEXT_CHAR_BUDGET`. e2e `copilot` **7/7** (module-scoped grounding + `[n]`→id; `resourceId` anchor surfaces a record w/o keyword match; **`llm:use` без `incident:read` → no-answer, no LLM call**; metadata-only `copilot.ask` audit; 403; 400 empty/unknown module; 503 disabled). Suite **67 suites / 471 tests** — all AI suites green (copilot 7/7, rag 6/6, search-semantic 7/7, vector 4/4); `tsc`/eslint clean. (Clean full-green flush captured at P5.4 = 66/464; the P5.5 serial full run was 65/67 where the only 2 failures were pre-existing **unrelated** suites — `rate-limit`, `documents-search-index` — whose hooks **timed out at 153s/900s** from local host exhaustion after many back-to-back runs, not regressions; both green on a healthy host.) **ADR-0071.** **Boundary:** real generation = manual live-smoke. `apps/api/src/modules/copilot/`. Follow-ons: GIS/Docs/Workflow copilots (registry entries + anchor loaders); action-capable (tool-calling + per-tool RBAC); web copilot panels; SSE streaming.

### P5.6 — Document intelligence (OCR + classification + extraction) ✅ COMPLETED (2026-06-03 — text extraction; classification/fields follow-on)
**Decisions (2026-06-03):** for the sovereign КЧС single-site/no-GPU reality — **Tesseract + PDF text-layer** (gated CPU seam, live boundary), **text only** (classification/fields = follow-on), **async via BullMQ** (like preview P2.13), **into OpenSearch `content` + P5.2 re-embed**.
**P5.6a (substrate) ✅:** `document_text` sidecar (migration 0042 + RLS two-GUC; `tenant_id`, `document_id` unique FK cascade, `content`, `char_count`, `status` done/empty, `extracted_at`) — sidecar (not a `documents` column) so large text never bloats list/get. Gated `TEXT_EXTRACTOR` seam (`text-extractor.ts` Noop + `createTextExtractor` on `DOC_EXTRACT_ENABLED`; `text-extractor.impl.ts` live-boundary — `pdf-parse` text-layer + `tesseract.js` OCR via non-literal dynamic import so the OCR toolchain stays out of the build/test deps). `DocumentExtractionService.extract` (runForTenant → loadReadyDoc → `storage.getObjectBytes` → `extractor.extract(bytes,mime,{ocrLang})` → cap `DOC_EXTRACT_MAX_CHARS` → upsert; 503 disabled / 404 not-ready) + `status`. `POST /v1/documents/:id/extract` (`document:write`, sync) + `GET /v1/documents/:id/text` (`document:read`). Contracts `document-text.ts`; config `DOC_EXTRACT_ENABLED`/`MAX_CHARS`/`OCR_LANG` (eng+rus). e2e `document-extraction` **7/7** (extract→store chars+status; idempotent upsert; empty; not-yet-extracted; 404; RBAC 403; 503 disabled — faked extractor, real S3). tsc/eslint clean.
**P5.6b (async + re-index) ✅:** gated BullMQ `EXTRACT_QUEUE` + `ExtractWorker` (gated+isTest-skip, dyn-import bullmq/ioredis → `extract`), mirroring the preview queue. `DocumentsService` **auto-enqueues** extraction on every finalize path (best-effort, alongside the preview enqueue; default `NoopExtractQueue` ⇒ no-op in dev/test). After storing text, `extract()` **best-effort re-indexes**: OpenSearch `indexDocument({…, content})` + vector re-embed with content (`IndexedDocument` + vector `DocLike` gained optional `content`; OpenSearch mapping/payload/`multi_match` + `textOf` include it) — one extraction makes keyword search, semantic (P5.3), RAG (P5.4), copilots (P5.5) content-aware; never fails extraction. e2e `document-extract-pipeline` **2/2** (re-index → OpenSearch `content` + a `document_embeddings` row; auto-enqueue on finalize). **Blast radius — 15 suites / 81 tests green serially** (documents×5, search×3, vector, rag, copilot, previews, extraction×2 — `content` optional everywhere ⇒ non-breaking); `tsc`/eslint clean. Suite **69 suites / 480 tests** (full-green flush recommended on an unloaded host — see ADR-0071). **ADR-0072.** **Boundary:** real PDF/Tesseract + BullMQ worker = manual live-smoke. `apps/api/src/modules/documents/`. Follow-ons: classification + structured fields; chunking (>8k); per-page/lang OCR; web extracted-text view + reindex-all-with-content.

### P5.7 — Multi-region active-active — ⛔ N/A (single-site reality)
**Decision (2026-06-03):** marked **not applicable** for this deployment. Active-active multi-region requires **≥2 datacentres**, but the established reality is **single-site** (server + backups co-located at the КЧС head office, no second datacentre — see P4.6/P4.7 single-site notes; same reason Linkerd and the GPU tier were dropped). The meaningful single-site analogue is **DR / warm-standby readiness** (backup freshness + restore drill + failover runbook + RPO/RTO + the replica/Sentinel profiles already in P3.13) — captured as a follow-on, not active-active. Per the user (AskUserQuestion 2026-06-03): skip to **P5.8**. Revisit only if a second sovereign site is ever provisioned.

### P5.8 — Sovereign / airgapped installers ✅ COMPLETED (2026-06-03)
**Decisions (2026-06-03):** AskUserQuestion locked **docker-save images tar + compose + scripts** (not an OCI registry mirror — fits the existing compose stack), **full stack** in one bundle (app + data plane + observability), **SHA-256 manifest + verify** (no cosign/GPG — no on-site key material). Three `infra/airgap/` tools + a runbook. **`build-bundle.sh`** (connected host): `git describe` version → builds from-source images (api/web/postgres/backup) → enumerates **every** image across `deploy-compose.yml` + `docker-compose.yml` + `observability-compose.yml` via `docker compose config --images` (deduped) → `docker save | gzip` → `images.tar.gz`; stages compose + `.env.example` + on-site scripts + `VERSION` + runbook; writes `MANIFEST.sha256`; packs `cmc-airgap-<version>.tar.gz` (+ outer `.sha256`); portable hashing. **`verify-bundle.sh`**: `sha256sum -c MANIFEST.sha256` integrity gate (refuses tampered bundles). **`install.sh`** (offline host): verify → `docker load` (offline) → ensure `.env` → start data plane → wait Postgres health → migrate (one-shot via api image, `MIGRATE_CMD` override) → `compose up -d` → `/health` smoke. Runbook `docs/runbooks/sovereign-airgap-install.md` (build→transfer→verify→configure→install→DR; gated AI flags stay off unless their on-host toolchain exists). Validation: `bash -n` clean on all three + image-enumeration dry-run against live compose (full resolved set incl. built `cmc/*`); infra/ops scripts validated by syntax+dry-run+runbook (like P3.13/P0.5), no app code touched → backend suite unaffected. **ADR-0073.** **Boundary:** real offline build→transfer→install drill = manual (multi-GB, Docker-on-host). Follow-ons: cosign/GPG signing (provenance); slim core+profiles build; delta upgrade bundles; backup-freshness/RPO-RTO (single-site DR from P5.7).

### P5.9 — Edge intelligence — ⛔ N/A (single-site / no-GPU)
**Decision (2026-06-03):** **not applicable.** Edge intelligence = distributed edge compute nodes running local inference. Reality is **single-site, no GPU** — one server at the КЧС head office; regions are a **data** split (P4.6), not a compute fabric. (Per AskUserQuestion 2026-06-03.) Revisit only if field/edge compute is ever provisioned; the offline-field analogue is the deferred P4.4 PWA.

### P5.10 — Federation — ⛔ N/A (single sovereign instance)
**Decision (2026-06-03):** **not applicable** for a single sovereign КЧС instance — no second instance/agency to federate with. (Per AskUserQuestion 2026-06-03.) Revisit only if cross-agency federation becomes a requirement.

### ✅ Full backend e2e — GREEN (2026-06-03)
Confirmed **70 suites / 484 tests, 0 failures** — run sharded serially (`jest --shard=1/3|2/3|3/3 --runInBand`: 24/184 + 23/147 + 23/153) to fit the constrained dev host. (The earlier full-**parallel** runs wedged/OOM'd on host contention — the *identical* suites all pass serially, confirming those were environmental, not code.) On a normally-resourced host/CI a single `pnpm --filter @cmc/api test:e2e` runs green. `tsc`/eslint clean across packages; web `tsc`/`lint`/`build` green.

### Horizon P5 — status (2026-06-03)
**AI substrate COMPLETE:** P5.1 LLM gateway, P5.2 vector pipeline, P5.3 semantic search, P5.4 RAG, P5.5 copilots, P5.6 document intelligence — all ✅. P5.8 sovereign/airgap installer ✅. **Distributed-scale items N/A** for single-site: P5.7 multi-region, P5.9 edge, P5.10 federation. Remaining real work redirected to the **single-site DR follow-on** (below).

### P5.DR — Single-site disaster-recovery readiness (reframed from P5.7) ✅ COMPLETED (2026-06-03)
**Decision (2026-06-03):** the single-site analogue of multi-region resilience — make backup freshness **observable** + document RPO/RTO + the restore drill (you can't fail over to a 2nd site, so the guarantee is "a fresh, restorable backup always exists"). `StorageService.listObjects` (new, `ListObjectsV2`). `BackupStatusService.status()` lists `postgres/*.dump` in the backups bucket → newest by `lastModified` → age vs RPO → `fresh`. `GET /v1/ops/backups/status` (`@Authorize monitoring:read`) → `{bucket, count, latestKey, latestAt, ageHours, rpoHours, fresh}`. Config `BACKUP_S3_BUCKET` (cmc-backups) + `BACKUP_RPO_HOURS` (36 — matches the P0.5 alert threshold). Platform-level (whole-DB dumps, not tenant-scoped). Contracts `backup.ts`; no DB/migration. Runbook `docs/runbooks/disaster-recovery.md` (RPO/RTO targets, freshness check, restore drill `pnpm db:restore`, full-host air-gap rebuild via P5.8, P3.13 warm-standby, what single-site DR is **not**). e2e `backup-status` **4/4** (fresh within RPO/ignores non-`.dump`; stale→not fresh; empty→count 0/nulls/not fresh; RBAC 403 — faked StorageService). `tsc`/eslint clean. **ADR-0074.** **Boundary:** real MinIO listing + restore drill = live/manual. Follow-ons: Prometheus `cmc_backup_age_hours` gauge + Alertmanager rule; scheduled freshness notification; periodic **test-restore** (restorability, not just freshness); WAL/PITR.
