...
| Visual builder (P3.8) | **In progress.** P3.8a: `workflows` table (DAG `definition` JSONB, RLS, migration 0025) + `workflow.ts` contracts (node-type union + `validateWorkflowDefinition`) + `workflow:*` + CRUD/validate. **P3.8b: generic interpreter** — one Temporal workflow (`workflow-interpreter.workflow.ts`, determinism-safe, local graph types) walks any DAG (start/end/delay/condition in-workflow; notify/create_incident activities); `workflow_runs` table (snapshot + status, migration 0026); `WorkflowsService.startRun`/`run` (snapshot + start, reused by P3.8c) + `POST /workflows/:id/run` + run-status; e2e (faked seam) + **live smoke** (real worker: start→notify→create_incident→end → completed + real side effects). Next: event triggers (P3.8c), React Flow editor (P3.8d / ADR-0053). Files: `apps/api/src/modules/workflows/`, `apps/api/src/modules/temporal/{workflows/workflow-interpreter.workflow,activities/workflow-interpreter.*}.ts`, `packages/db/src/schema/workflow{,-runs}.ts`, `packages/contracts/src/workflow.ts` |
