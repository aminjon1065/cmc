...
function SourceBadge({ source }: { source: SearchResult["source"] }) {
  ...
  return (
    <span
      ...
      title={
        isOs ? "Served by OpenSearch" : "Served by Postgres full-text search"
      }
    >
      {source}
    </span>
...
export default async function SearchPage({
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div className="cmc-label mb-1">Intelligence · Search</div>
          <div
          ...
          <div className="mt-1 text-[11.5px]" style={{ color: "var(--c-fg-3)" }}>
            {q ? `${total} result${total === 1 ? "" : "s"} for “${q}”` : "Across incidents, cases, and documents."}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 p-5">
        ...
        {q.length === 0 ? (
          ...
          >
            Type a query above to search across the platform.
          </div>
        ) : result && !result.ok ? (
          <div
          ...
          TYPE_ORDER.filter((t) => groups.has(t)).map((type) => (
            <div key={type} className="cmc-card">
              <div
                ...
                style={{ borderBottom: "0.5px solid var(--c-line-2)" }}
              >
                <span className="cmc-label">{TYPE_LABEL[type]}</span>
